 ...
 * Scanner
 *
 * Utility for scanning a directory for added, removed and changed files and reporting these events
 * via registered Listeners.
 *
 */
public class DirectoryScanner {

    public static void watch(int scanIntervalSeconds, List<File> dirs, DirectoryScanner.Listener... listeners) throws IOException {
        DirectoryScanner directoryScanner = new DirectoryScanner();
        directoryScanner.setReportExistingFilesOnStartup(false);
        directoryScanner.setScanInterval(scanIntervalSeconds);
        directoryScanner.setScanDirs(dirs);
        directoryScanner.setRecursive(true);
        for (DirectoryScanner.Listener listener : listeners) {
            directoryScanner.addListener(listener);
        }
        directoryScanner.doStart();
        System.out.println("Watching directories recursively:");
        for (File dir : dirs) {
            System.out.println("  " + dir);
        }
        System.out.println("Scan interval seconds: " + scanIntervalSeconds);
    ...
    /**
     * Apply a filter to files found in the scan directory. Only files matching the filter will be
     * reported as added/changed/removed.
     *
     * @param filter
     */
     ...
     *
     * @param reportExisting if true, all files found on initial scan will be reported as being
     * added, otherwise not
     */
